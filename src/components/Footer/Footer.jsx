import { Home, Mail, Phone } from "lucide-react";
import React from "react";
import Link from "next/link";
import Image from "next/image";
import priceBadge from "@assets/price-badge-yellow.webp";
import {
  contactInfo,
  footerLinks,
  portfolioLink,
  socialLinks,
} from "@constants/links";

const socialIcons = {
  telegram: (
    <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden>
      <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.161c-.18 1.897-.962 6.502-1.359 8.627-.168.9-.5 1.201-.82 1.23-.697.064-1.226-.461-1.901-.903-1.056-.693-1.653-1.124-2.678-1.8-1.185-.781-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.139-5.062 3.345-.479.329-.913.489-1.302.481-.428-.009-1.252-.242-1.865-.44-.752-.244-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.831-2.529 6.998-3.014 3.333-1.386 4.025-1.627 4.477-1.635.099-.002.321.023.465.141a.506.506 0 0 1 .171.325c.016.093.036.306.02.472z" />
    </svg>
  ),
  whatsapp: (
    <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.435 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z" />
    </svg>
  ),
  github: (
    <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden>
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  ),
  linkedin: (
    <svg viewBox="0 0 24 24" className="w-6 h-6 fill-current" aria-hidden>
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  ),
};

const Footer = () => {
  return (
    <footer className="bg-[#1A1A1A] text-white text-sm relative">
      <Image
        src={priceBadge}
        alt="price-badge-yellow"
        width={120}
        height={80}
        className="absolute object-cover bottom-14 right-2"
      />
      <div className="container mx-auto grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 py-2 gap-5">
        <div className="border-b sm:border-b md:border-none pb-8 mt-10">
          <Image
            src="/logo.webp"
            alt="footer-logo"
            width={160}
            height={120}
            className="pb-4"
          />
          <p className="text-gray-500 text-sm leading-relaxed">
            Elevate bites, build community, deliver culinary excellence with
            FastFood TNC.
          </p>
          <ul className="mt-4 space-y-2.5 text-sm">
            <li className="flex items-start gap-2">
              <Home className="mt-0.5 shrink-0 w-4 h-4" />
              <span className="text-gray-500 hover:text-white duration-300 break-words">
                {contactInfo.address}
              </span>
            </li>
            <li className="flex items-start gap-2">
              <Phone className="mt-0.5 shrink-0 w-4 h-4" />
              <a
                href={contactInfo.phoneHref}
                className="text-gray-500 hover:text-white duration-300"
              >
                {contactInfo.phone}
              </a>
            </li>
            <li className="flex items-start gap-2">
              <Mail className="mt-0.5 shrink-0 w-4 h-4" />
              <a
                href={contactInfo.emailHref}
                className="text-gray-500 hover:text-white duration-300 break-all min-w-0"
              >
                {contactInfo.email}
              </a>
            </li>
          </ul>
        </div>

        <div className="border-b sm:border-b md:border-none py-8 mt-1 ms-0 md:ms-12">
          <h3 className="text-lg font-bold text-[#FFB700] mb-3">
            Quick Links
          </h3>
          <ul className="space-y-2 text-sm">
            {footerLinks.map((link) => (
              <li key={link.label}>
                <Link
                  href={link.href}
                  className="text-gray-500 hover:text-white duration-300"
                >
                  {link.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        <div className="border-b sm:border-b md:border-none py-8">
          <h3 className="text-lg font-bold text-[#FFB700] mb-3">
            Opening Hours
          </h3>
          <ul className="space-y-2 text-sm">
            <li>
              Monday to Friday :{" "}
              <span className="text-gray-500 hover:text-white duration-300 cursor-pointer">
                10:00 AM to 12:00 PM
              </span>
            </li>
            <li>
              Saturday :{" "}
              <span className="text-gray-500 hover:text-white duration-300 cursor-pointer">
                12:00 PM to 6:00 PM
              </span>
            </li>
          </ul>
        </div>

        <div className="py-8">
          <h3 className="text-lg font-bold text-[#FFB700] mb-3">
            Newsletter Subscribe
          </h3>
          <p className="pb-3 text-sm">
            Stay In the Loop :{" "}
            <span className="text-gray-500 hover:text-white duration-300 cursor-pointer">
              Unlock Exclusive Offers, Culinary Insights, and More.
            </span>
          </p>
          <input
            type="text"
            className="relative w-full md:w-2/2 p-2 mt-2 mb-4 border text-black text-sm outline-none"
            placeholder="Enter your email"
          />
          <ul className="flex space-x-4">
            {socialLinks.map((link) => (
              <li key={link.name}>
                <Link
                  href={link.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={link.label}
                  className="text-white hover:text-[#FFB700] duration-300 inline-flex [&>svg]:w-5 [&>svg]:h-5"
                >
                  {socialIcons[link.name]}
                </Link>
              </li>
            ))}
          </ul>
        </div>
      </div>

      <div className="text-center">
        <p className="border-t border-slate-700 py-2 text-center text-xs">
          © All Rights Reserved
          <Link
            href={portfolioLink.href}
            rel="noopener noreferrer"
            target="_blank"
            className="text-decoration-none text-[#FFB700]"
          >
            <span className="ms-1">{portfolioLink.label}.</span>
          </Link>
        </p>
      </div>
    </footer>
  );
};

export default Footer;
